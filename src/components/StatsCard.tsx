...
const StatsCard: React.FC<StatsCardProps> = ({ title, value, icon: Icon, color, subtitle }) => {
  ...
  return (
    <div className={`group ${colors.card} backdrop-blur-sm p-6 rounded-3xl shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-2xl ${colors.glow} border cursor-pointer relative overflow-hidden`}>
      {/* Background Glow Effect */}
      <div className="absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-300">
        <div className={`absolute inset-0 ${colors.bg} rounded-3xl`}></div>
      </div>
      
      <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <div className={`${colors.bg} p-4 rounded-2xl shadow-lg ${colors.shadow} group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 flex-shrink-0`}>
          <Icon className={`h-6 w-6 ${colors.icon}`} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-600 mb-1 truncate group-hover:text-gray-700 transition-colors duration-200">
            {title}
          </p>
          <p className={`text-2xl font-bold ${colors.text} mb-1 group-hover:scale-105 transition-all duration-300 truncate`}>
